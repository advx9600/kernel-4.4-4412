...
//		ret = ilitek_i2c_read(i2c.client, ILITEK_TP_CMD_READ_SUB_DATA, buf, 5);//jeff
		if(ret < 0){
			return ret;
		}
		for (i = 0; i < 64; i++)
	...
	const struct i2c_device_id *id)
{
	...
	gpio_request(FT_WAKE_PORT,"tp-reset");
	...
	void)
{
	...
	printk("%s, client.adapter: 0x%X\n", __func__, (unsigned int)i2c.client->adapter);
	printk("%s, client.driver: 0x%X\n", __func__, (unsigned int)i2c.client->driver);
	printk("%s, client.irq: 0x%X\n", __func__, (unsigned int)i2c.client->irq);
