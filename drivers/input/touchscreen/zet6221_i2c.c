...
#define TS_INT_GPIO		EXYNOS4_GPX0(3)//RK29_PIN0_PA2//S3C64XX_GPN(9)  /*s3c6410*/
//#define TS1_INT_GPIO        AT91_PIN_PB17 /*AT91SAM9G45 external*/
//#define TS1_INT_GPIO        AT91_PIN_PA27 /*AT91SAM9G45 internal*/
#define TS_RST_GPIO		EXYNOS4_GPX0(6)//EXYNOS4212_GPM3(3)//RK29_PIN6_PC3//S3C64XX_GPN(10)

#define EINT_NUM	IRQ_EINT(3)
#define GPIO_LOW 0
#define GPIO_HIGH 1
#define TPINFO	0
#define X_MAX	1280//1024//800//1024
#define Y_MAX	800//768//480//576
...
/*
*   test	 i2c communicate 
* 	
*   return valule :   success : 1 ;   failed :     0
*/

u8 zet6221_ts_test_i2c(struct i2c_client *client)
{
	u8 ts_report_cmd[1] = {0xb2};
...
static void zet6221_ts_work(struct work_struct *_work)
{
	...
	struct i2c_client *tsclient1;

	//printk("=<%s>=<bufLength : %d >\n",__func__,bufLength);
	...
	*/
	...
	#if 0  // only for debug  @ rpdzkj
	for(i = 0;i < 3;i++){
		x[i] = (int)(x[i]*X_MAX)/1790;
		y[i] = (int)(y[i]*Y_MAX)/1152;
	}
	printk("========(%d,%d),(%d,%d),(%d,%d)   ret : 0x%x \n",x[0],y[0],x[1],y[1],x[2],y[2],ret);
  	#endif
	...
	if(ret == 0x3C)
	{
		...
		//	printk( "valid=%04X pressure[%d]= %d x= %d y= %d\n",points , i, pressure,x[i],y[i]);
...
no_finger:
		...
		enable_irq(EINT_NUM);
	  	...
	  	enable_irq(EINT_NUM);  // add by rp210 @20140707
...
//bootloader
zet_download:
#if 0  // msk by rp210 , because TP has firmware @ 20140707
...
#if 1   // add by rp210 for test i2c communication 
/*
	for(count = 0 ; count < 4 ; count++){		
		test = zet6221_i2c_write_tsdata(client, NULL, 0);		
		if(test > 0){				
			printk("===<%s>==<i2c ok>==\n",__func__);			
			break;	
		}
	...
	if(test <= 0){		
		printk("===<%s>===<i2c test error>====\n",__func__);
		goto i2c_communicate_error;
	}
*/	
	test = zet6221_ts_test_i2c(client);
	if(test == 0)
		goto i2c_communicate_error;
...
#ifndef REF_FT	 // mod by rp210 @20140707
	//s3c6410
